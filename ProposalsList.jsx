...
export const ProposalsList = ({ onSelectProposal, onOpenInspector }) => {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="relative overflow-hidden rounded-3xl border border-sky-500/20 bg-gradient-to-br from-slate-900/90 via-slate-950 to-indigo-950/40 p-6 sm:p-10 shadow-[0_0_35px_rgba(56,189,248,0.1)]">
        ...
        <div className="relative z-10 max-w-3xl space-y-4">
          ...
          <h1 className="font-heading text-3xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight text-white leading-tight">
            Vote Freely. <span className="midnight-gradient-text">Keep It Confidential.</span>
          </h1>
          ...
          <div className="pt-2 flex flex-wrap items-center gap-3">
            <Button
              ...
              className="bg-gradient-to-r from-sky-500 to-indigo-600 hover:from-sky-400 hover:to-indigo-500 text-white font-mono font-bold text-xs px-5 py-2.5 rounded-xl flex items-center space-x-2 shadow-[0_0_20px_rgba(56,189,248,0.3)]"
            >
            ...
            <Button
              ...
              className="border-slate-700 bg-slate-900/60 hover:bg-slate-800 text-slate-300 font-mono text-xs px-4 py-2.5 rounded-xl flex items-center space-x-2"
            >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-8 pt-6 border-t border-slate-800/80">
          <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800/80">
            <div className="text-[10px] font-mono text-slate-400 uppercase">Active Proposals</div>
            <div data-testid="stat-active-proposals" className="text-xl font-heading font-bold text-white mt-0.5">
              {proposals.filter((p) => p.status === 'OPEN').length}
            </div>
          </div>
          <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800/80">
            <div className="text-[10px] font-mono text-slate-400 uppercase">Total Shielded Ballots</div>
            <div data-testid="stat-total-ballots" className="text-xl font-heading font-bold text-sky-400 mt-0.5">
              {proposals.reduce((sum, p) => sum + (p.total_ballots_cast || 0), 0)}
            </div>
          </div>
          <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800/80">
            <div className="text-[10px] font-mono text-slate-400 uppercase">Circuit Constraints</div>
            <div className="text-xl font-heading font-bold text-purple-400 mt-0.5">3,840 R1CS</div>
          </div>
          <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800/80">
            <div className="text-[10px] font-mono text-slate-400 uppercase">Deployed Preprod</div>
            <div className="text-xs font-mono text-emerald-400 font-semibold mt-1 truncate">
              02008f1b...ecb2
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-4 items-stretch md:items-center justify-between">
          ...
          <div className="relative flex-1 max-w-md">
            ...
            <Input
              ...
              className="pl-10 bg-slate-900/80 border-slate-800 text-white placeholder:text-slate-500 font-sans text-xs sm:text-sm rounded-xl focus:border-sky-500"
            />
          </div>
          ...
          <div className="flex items-center space-x-2 overflow-x-auto pb-1 sm:pb-0">
            ...
            <Button
              ...
              className="text-slate-400 hover:text-sky-400 text-xs px-2.5"
            >
        ...
        <div className="flex items-center space-x-2 overflow-x-auto pb-2 scrollbar-none">
          {categories.map((cat) => (
            <button
              ...
              className={`whitespace-nowrap px-3.5 py-1.5 text-xs font-mono rounded-full border transition-all ${
      ...
      {loading ? (
        ...
        <div
          ...
          className="py-16 text-center space-y-3 rounded-2xl border border-slate-800/80 bg-slate-950/40 p-8"
        >
          <Vote className="w-10 h-10 text-slate-600 mx-auto" />
          <div className="font-heading text-base text-slate-300">No Proposals Found</div>
          ...
          {filteredProposals.map((prop) => {
            ...
            return (
              <div
                ...
                className="rounded-2xl border border-slate-800/80 bg-slate-900/60 hover:bg-slate-900/90 transition-all p-5 flex flex-col justify-between space-y-4 hover:border-sky-500/30 hover:shadow-[0_0_20px_rgba(56,189,248,0.1)]"
              >
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    <Badge
                      variant="outline"
                      className="bg-sky-500/10 text-sky-400 border-sky-500/30 text-[10px] font-mono uppercase"
                    >
                      {prop.category}
                    </Badge>
                    <Badge
                  ...
                  {/* Title & Description */}
                  <div>
                    <h3 className="font-heading text-base font-bold text-white line-clamp-2 hover:text-sky-300 cursor-pointer">
                      {prop.title}
                    </h3>
                  ...
                  <div className="space-y-1.5 pt-2">
                    ...
                    <div className="flex items-center justify-between text-[10px] font-mono text-slate-400 pt-0.5">
                      <span className="text-emerald-400">Yes: {prop.tally_yes} ({yesPct}%)</span>
                      <span className="text-rose-400">No: {prop.tally_no} ({noPct}%)</span>
                ...
                <div className="pt-3 border-t border-slate-800/80 space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <Button
                      ...
                      className="border-slate-800 hover:bg-slate-800 text-slate-300 text-xs font-mono flex items-center justify-center gap-1"
                    >
                    ...
                    <Button
                      ...
                      className={`text-xs font-mono font-bold flex items-center justify-center gap-1 ${
                        isOpen
                          ? 'bg-gradient-to-r from-sky-500 to-indigo-600 hover:from-sky-400 hover:to-indigo-500 text-white shadow-[0_0_10px_rgba(56,189,248,0.3)]'
