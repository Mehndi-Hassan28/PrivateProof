  ...
  return (
    <div className="space-y-8" data-testid="test-suite-view">
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6 space-y-3 font-mono text-xs">
        ...
        <pre
          data-testid="ci-workflow-file-view"
          className="p-3.5 rounded-xl bg-slate-950 border border-slate-800/80 text-slate-300 text-[11px] overflow-x-auto"
        >
{`name: PrivateVote CI/CD Pipeline
...
jobs:
  build-and-test:
    ...
    steps:
      ...
      - name: Setup Compact Toolchain v0.23
        run: ./scripts/compile_compact.sh contract/private_vote.compact
      - name: Run Backend Pytest Suite
        run: cd backend && pytest tests/
      - name: Validate Managed ZK Circuits & Keys
        run: test -f managed/private_vote/keys/private_vote.prover
